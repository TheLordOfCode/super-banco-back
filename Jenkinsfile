    ...
    stage('Checkout') {
      steps{
		...
		checkout([
			...
			submoduleCfg: [],
			userRemoteConfigs: [[
			credentialsId: 'GitHub_TheLordOfCode',
			url:'https://github.com/TheLordOfCode/super-banco-back'
			]]
		])
      }
    }
  ...
  post {
    ...
    failure {
      echo 'This will run only if failed'
       mail (to: 'juan.quevedo@ceiba.com.co',subject: "Failed Pipeline:${currentBuild.fullDisplayName}",body: "Something is wrong with ${env.BUILD_URL}")
    }
